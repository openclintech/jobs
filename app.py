...
def filter_data(data, remote_only, include_no_comp, min_comp, max_comp, keywords, companies, cities, states):
    # Start with all data if including jobs without compensation details
    if include_no_comp:
        filtered = data.copy()
    else:
        # Only include jobs with specified compensation details
        filtered = data.dropna(subset=['compensation_min', 'compensation_max'])

    # Apply compensation range filter to jobs with specified compensation
    # This logic ensures we don't exclude jobs without compensation details if include_no_comp is True
    if not include_no_comp or (min_comp > 0 or max_comp < filtered['compensation_max'].max()):
        filtered = filtered[(filtered['compensation_min'] >= min_comp) & (filtered['compensation_max'] <= max_comp) | filtered['compensation_min'].isna() | filtered['compensation_max'].isna()]
